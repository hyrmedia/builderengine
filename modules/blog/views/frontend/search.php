                     ...
                     <?php foreach($posts as $post):?>
							<li class="masonry-item">
								<div class="item">
									...
									<?php if($post->image != ''): ?>
									<figure>
										<a href="/blog/post/<?=$post->slug?>"><img src="<?=$post->image?>" class="img-responsive" alt="" /></a>
									</figure>
									<?php endif; ?>
									<?php
						...
						<div class="widget">
							<h4>TAGS</h4>
							 ...
							 $available_tags = array();
					           ...
					           $num_tags = count($available_tags);
							   
								if($set_limit > $num_tags)
								{
								   $limit = $num_tags;
								}
								else
	                            ...
	                            $available_tags = array_slice($available_tags,0,$limit);
							?>
							    <?php foreach($available_tags as $tag):?>
                                    <a class="label label-default light" href="<?=base_url('blog/search/'.$tag)?>"><i class="fa fa-tags"></i> <?=$tag?></a>
							    <?php endforeach?>
